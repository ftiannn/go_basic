...
func main() {
	...
	s2 := `I Like \n Go!` // raw string -> I Like \n Go!
	fmt.Println(s2)

	fmt.Println("Price: 100\nBrand: Nike") //Price: 100 <br> Brand: Nike

	// Print C:\Users\xxxx
	fmt.Println(`C:\Users\xxxx`)
	fmt.Println("C:\\Users\\xxxx")

	var s3 = "I love " + "Hello" + " World"
	fmt.Println(s3 + "!")

	fmt.Println("Element at index 0:", s3[0]) //Return ASCII Dec -> 73

	// s3[5] = 'x' --> error: strings are immutable and can't be changed
	...
	str1 := "abcdefghijkl"
	fmt.Println("\nstr1", str1[2:5]) // -> cde, bytes from 2 (included) to 5 (excluded)

	str2 := "中文维基是世界上"
	fmt.Println("str2", str2[0:2]) // -> � - the unicode representation of bytes from index 0 and 1.
